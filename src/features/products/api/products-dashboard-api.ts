...
const mapProduct = (product: ProductApiResponse): Product => {
  ...
  return {
    ...
    variants: variants.map((variant, index) => ({
      ...
      sortOrder: variant.sortOrder ?? variant.SortOrder ?? index, isDefault: variant.isDefault ?? variant.IsDefault ?? false,
    })).sort((first, second) => first.sortOrder - second.sortOrder),
    categories: categories.map((category) => ({ id: category.id ?? category.Id ?? '', name: category.name ?? category.Name ?? '' })),
...
const normalizeVariants = (variants?: ProductVariantInput[]): ProductVariantInput[] => {
  const normalized = (variants ?? []).map((variant, index) => ({ ...variant, name: variant.name.trim(), sortOrder: variant.sortOrder ?? index, isDefault: Boolean(variant.isDefault) })).filter((variant) => variant.name.length > 0);
  if (normalized.length === 0) return [];
  const defaultIndex = normalized.findIndex((variant) => variant.isDefault);
  return normalized.map((variant, index) => ({ ...variant, isDefault: defaultIndex >= 0 ? index === defaultIndex : index === 0 }));
};
...
const appendVariantsToFormData = (formData: FormData, variants?: ProductVariantInput[]): void => {
  ...
  const variantsJson = normalizeVariants(variants).map((variant, index) => {
    const item: Record<string, unknown> = { name: variant.name, sortOrder: variant.sortOrder ?? index, isDefault: Boolean(variant.isDefault) };
    if (variant.id) item.id = variant.id;
    if (variant.imageFile) { item.imageIndex = variantImages.length; variantImages.push(variant.imageFile); }
    else if (variant.imageId) item.imageId = variant.imageId;
    else if (variant.imagePath) item.imagePath = variant.imagePath;
    return item;
  });
...
export const productsDashboardApi = {
  async getProducts(filters: ProductsFilter, pagination: PaginationParams): Promise<PaginatedData<Product>> {
    if (env.enableMockApi) {
      return legacyProductsApi.getProducts({ ...filters, categoryId: filters.categoryId !== 'all' ? filters.categoryId : filters.categoryIds?.[0] ?? 'all' }, pagination);
    }
  ...
  async getProduct(productId: string): Promise<Product> {
    if (env.enableMockApi) return legacyProductDetailApi.getProduct(productId);
  ...
  async getProductsBrief(): Promise<ProductBrief[]> {
    if (env.enableMockApi) {
      const page = await legacyProductsApi.getProducts({ search: '', categoryId: 'all' }, { page: 1, pageSize: 100 });
      return page.items.map((product) => ({ id: product.id, name: product.name, parCode: product.code, brand: product.brand, type: product.type, weight: product.weight, weightUnit: product.weightUnit }));
    }
  ...
  async createProduct(payload: CreateProductInput): Promise<void> {
    ...
    if (env.enableMockApi) { await legacyProductsApi.createProduct(parsed); return; }
  ...
  async updateProduct(payload: UpdateProductInput): Promise<void> {
    ...
    if (env.enableMockApi) { await legacyProductsApi.updateProduct(parsed); return; }
    ...
    if (shouldUseMultipart) {
      ...
      if (parsed.clearCategories) formData.append('ClearCategories', 'true');
      else if (categoryIdsProvided) {
        if (normalizedCategoryIds.length > 0) normalizedCategoryIds.forEach((categoryId) => formData.append('CategoryIds', categoryId));
        else formData.append('ClearCategories', 'true');
      }
    ...
    if (parsed.clearCategories) body.clearCategories = true;
    else if (categoryIdsProvided) {
      if (normalizedCategoryIds.length > 0) body.categoryIds = normalizedCategoryIds;
      else body.clearCategories = true;
    }
  ...
  async deleteProduct(productId: string): Promise<void> {
    if (env.enableMockApi) { await legacyProductsApi.deleteProduct(productId); return; }
    await apiClient.delete(`/api/Products/${productId}`);
  },

  async importProducts(file: File): Promise<unknown> {
    if (env.enableMockApi) return { imported: 0, mock: true };
    const formData = new FormData();
